# Architecture & Resource Documentation Index

- [Architecture Overview](README.md)
- [CI/CD & Public Repository Security](ci-cd-security.md)
- [GitHub Actions Workflows](github-actions.md)
- [Azure DevOps Pipelines](azure-pipelines.md)
- [Resource Groups](resource-groups.md)
- [Virtual Network & Networking](virtual-network.md)
- [User-Assigned Managed Identities](managed-identity.md)
- [Role-Based Access Control](role-based-access-control.md)
- [Azure Storage Account](storage-account.md)
- [Azure Key Vault](key-vault.md)
- [Azure OpenAI Service](azure-openai.md)
- [Private Endpoints](private-endpoints.md)
- [Azure AI Foundry Hub](ai-hub.md)
- [Azure AI Foundry Project](ai-project.md)
- [Log Analytics Workspace](log-analytics.md)
- [Windows Jumpbox VM](virtual-machine.md)
- [Python Chat Application](chat-application.md)

---
- [Root Index](../index.md)
