...
- **RDP allowlist.** Environment YAML addresses populate stable `allow-rdp-user`; the detected deployment/connection IP populates temporary `allow-rdp-deployer`; `deny-rdp-all` remains last. Use `../main.ps1 dev-connect` or `../main.ps1 uat-connect` to refresh access without deployment. The weekly Automation runbook removes only `allow-rdp-deployer`.
- **Connect password display.** Because Key Vault public access is disabled and ARM never returns secret values, `*-connect` uses VM Run Command to read `vm-admin-password` from the private endpoint with the VM managed identity. Only the marked result is printed to the invoking console; the script does not use repository logging or persist the returned value.
- **Selective cleanup.** `*-clean` enumerates only tag-validated environment resource groups, deletes their resources in dependency order, and preserves the environment Key Vault and VM OS disk. Resource-group containers remain.
