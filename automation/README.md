...
|---|---|---|
| `delete-nsg-rule.ps1` | Idempotently deletes only the temporary `allow-rdp-deployer` NSG rule | Weekly at `rdpDeployerCleanupScheduleTime` |
| `schedule-vm-start.ps1` | Starts the environment jumpbox idempotently by using the Automation user-assigned managed identity | Daily at `vmStartScheduleTime` |
