...
| Azure MCP server usage | Live subscription reads turn deployment failures, model-array SKU/quota/region questions, RBAC gaps, and flag drift into evidence instead of guesses, and pair with the Bicep MCP server for accurate fixes. | `instructions/azure-mcp-server.instructions.md`, registered in `.mcp.json` and `.vscode/mcp.json` |
